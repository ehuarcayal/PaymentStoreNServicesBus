using CreditCards.Infrastructure.Mappings;
...
namespace CreditCards.NSBEndpoint
{
    class Program
    {
        static async Task Main()
        {
            ...
            var endPointName = "CreditCards.NSBEndpoint";
            Console.Title = endPointName;
            var endpointConfiguration = new EndpointConfiguration(endPointName);
            var transport = endpointConfiguration.UseTransport<RabbitMQTransport>();
            transport.UseConventionalRoutingTopology();
            string rabbitmqUrl = System.Environment.GetEnvironmentVariable("RABBITMQ_PCF_NSB_URL");
            transport.ConnectionString(rabbitmqUrl);
            string mysqlConnectionString = Environment.GetEnvironmentVariable("MYSQL_STRCON_CORE_CREDITCARDS");
        ...
        static Configuration AddFluentMappings(Configuration nhConfiguration, string connectionString)
        {
            return Fluently
                ...
                .Mappings(cfg =>
                {
                    cfg.FluentMappings.AddFromAssembly(typeof(CreditCardMap).Assembly);
